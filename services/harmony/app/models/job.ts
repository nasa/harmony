...
import { getWorkflowStepsByJobId } from './workflow-steps';
...
export class Job extends DBRecord implements JobRecord {
  ...
  */
  ...
  async updateProgress(tx: Transaction): Promise<void> {
    ...
    let sum = steps.reduce((sum: number, step: WorkflowStep) => sum + step.progress_weight * step.progress, 0);
    sum = Math.max(0, sum);
    // Only allow progress to be set to 100 when the job completes and don't let progress go
    // backwards
    const progress = Math.min(Math.floor(sum / sumOfWeights), 99);
