...
//   let req, res, next, getRetryCountsStub;
//   beforeEach(() => {
//     // Mock request object
//     req = {
//       query: {},
...
//     // Mock next function
//     next = sinon.stub();
//     // Stub the getRetryCounts function
//     getRetryCountsStub = sinon.stub(getRetryCounts);
//   });
//   afterEach(() => {
//     sinon.restore();
//   });
...
//       const mockRetryData = {
//         0: 100,  // 100 successful items
//         1: 20,   // 20 items retried once
//         2: 10,   // 10 items retried twice
//         3: 5,    // 5 items retried three times
//         4: 3,    // 3 items retried four times
//         5: 2,     // 2 items retried five times
//       };
//       getRetryCountsStub.resolves(mockRetryData);
//       req.accepts.returns('json');
//       await getRetryStatistics(req, res, next);
//       expect(getRetryCountsStub).to.have.been.calledOnceWith(sinon.match.any, 60);
...
//         totalRetries: 80,       // 0*100 + 1*20 + 2*10 + 3*5 + 4*3 + 5*2
//         percentSuccessful: '71.4%',  // 100/140 * 100
//         percentRetried: '28.6%',      // 40/140 * 100
//       });
//     });
...
//       req.accepts.returns('json');
//       await getRetryStatistics(req, res, next);
//       expect(getRetryCountsStub).to.have.been.calledWith(sinon.match.any, 120);
...
//       req.accepts.returns('json');
//       await getRetryStatistics(req, res, next);
//       expect(getRetryCountsStub).to.have.been.calledWith(sinon.match.any, 90);
//     });
//   });
//   describe('Edge cases', () => {
...
//       expect(result.percentSuccessful).to.equal('55.6%'); // 10/18 * 100
//       expect(result.percentRetried).to.equal('44.4%'); // 8/18 * 100
//     });
//     it('should handle invalid numMinutes parameter', async () => {
//       const mockRetryData = { 0: 10, 1: 5, 2: 3, 3: 2, 4: 1, 5: 0 };
//       getRetryCountsStub.resolves(mockRetryData);
//       req.query = { numMinutes: 'invalid' };
//       req.accepts.returns('json');
//       await getRetryStatistics(req, res, next);
//       // Should default to 60 when parseInt returns NaN
//       expect(getRetryCountsStub).to.have.been.calledWith(sinon.match.any, 60);
//     });
//   });
...
//       await getRetryStatistics(req, res, next);
//       expect(res.render).to.have.been.calledOnceWith('retry-stats', {
//         numMinutes: 60,
//         counts: [
...
//       await getRetryStatistics(req, res, next);
//       expect(req.context.logger.error).to.have.been.calledWith(error);
...
//       await getRetryStatistics(req, res, next);
//       expect(req.context.logger.info).to.have.been.calledWith(
...
// import { getRetryCounts } from '../app/models/work-item';

// // Set up sinon-chai
// use(sinonChai);

// describe('getRetryStatistics', () => {
//   let req: any, res: any, next: any, getRetryCountsStub: sinon.SinonStub;
