  ...
  describe('main', function () {
    ...
    describe('when the output directory exists', function () {
      ...
      it('outputs the result data to catalog.json in the directory', async function () {
        const catalog = await defaultObjectStore().getObject(resolve(tmpDir, 'catalog.json'));
      ...
      it('outputs the result data to item.json in the directory', async function () {
        const itemContext = JSON.parse(await defaultObjectStore().getObject(resolve(tmpDir, 'item.json')));
        ...
        expect(itemContext.assets['Giovanni URL'].href).to.equal('https://api.giovanni.uat.earthdata.nasa.gov/proxy-timeseries?data=GPM_3IMERGHH_06_precipitationCal&location=%5B-3.8%2C0.76%5D&time=2020-01-01T00%3A00%3A00%2F2020-01-01T03%3A00%3A00');
