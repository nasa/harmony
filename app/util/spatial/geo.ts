...
/**
 *  Given two points, returns their midpoint along the shortest great circle between them.
 * The two points may not be antipodal, since such a midpoint would be undefined
 * @param point1 - The first point on the great circle
 * @param point2 - The second point on the great circle
 * @returns interpolate - The midpoint
 */
export function gcInterpolate(point1: LatLng, point2: LatLng): LatLng {
  let p1 = point1;
  let p2 = point2;
 ...
 *
 * @param latlng1 - starting point
 * @param latlng2  - ending point
 */
function course(latlng1: LatLng, latlng2: LatLng): number {
 ...
 * @param a1 - the start angle
 * @param a2 - the end angle
 * @returns delta - the difference between the start and end angles
 */
 ...
 * @param angles - A sequential set of angles
 * @returns direction - the rotation of the angles as described above
 */
function rotationDirection(angles: number[]): number {
...
/**
 * Calculates area within the given latlngs
 * @param origLatlngs - An array of latitude/longitude pairs
 * @returns area - the area within the given latlngs
 */
export function calculateArea(origLatlngs: LatLng[]): number {
...
/**
 * Converts LatLngs
 * @param latlngs - An array of latitudie/longitude pairs
 * @returns an array of converted LatLng
 */
function convertLatLngs(latlngs: LatLng[]): LatLng[] {
  const result: LatLng[] = [];
 ...
 *
 * @param latlng0 - the start LatLng
 * @param latlng1 - the end LatLng
 * @returns crossing - the LatLng where the arc crosses the antimeridian or null if it does
 *   not cross
 */
function antimeridianCrossing(latlng0: LatLng, latlng1: LatLng): LatLng {
  const arc = new Arc(Coordinate.fromLatLng(latlng0), Coordinate.fromLatLng(latlng1));
  return arc.antimeridianCrossing()?.toLatLng();
}
 ...
 * @param latlngs - an array of LatLng representing a ring of points
 * @returns the original list in counter-clockwise order
 */
function makeCounterClockwise(latlngs: LatLng[]): LatLng[] {
 ...
 *
 * @param latLngs - a list of lists of LatLng
 * @returns polygon - an object with the interiors and boundaries of the result
 */
export function dividePolygon(latLngs: LatLng[][]): SplitPoly {
  let newLatLngs: LatLng[];
  let i;
  let lat;
  let lng;
  let asc;
  ...
  for (j = 0, i = j; j < split.length; j += 1, i = j) {
    ...
    if ((interior.length > 2) && (Math.abs(latlng.lng) === 180) && (Math.abs(next.lng) === 180)) {
      ...
      if (containsNorthPole && (latlng.lat === maxCrossingLat)) {
        ...
        inc = lng < 0 ? 90 : -90;
        for (i = 0; i <= 4; i += 1) {
          interior.push({ lat: 90, lng: lng + (i * inc) });
        }
      }
  ...
  interiorStack.forEach((inter) => interiors.push(inter));
  ...
  if (containsNorthPole && containsSouthPole && !hasInsertions) {
    interior = [];
    for (i = 0; i <= 4; i += 1) {
      interior.push({ lat: 90, lng: -180 + (i * 90) });
    }
    for (i = 0; i <= 4; i += 1) {
      interior.push({ lat: -90, lng: 180 - (i * 90) });
    }
 ...
 */
export function getShape(points: string): LatLng[] {
  const pointArray = _.chunk(points.split(' '), 2);
  return pointArray.map((point: [string, string]) => {
    const [lat, lng] = point.map(parseFloat);
    return { lat, lng };
  });
}
